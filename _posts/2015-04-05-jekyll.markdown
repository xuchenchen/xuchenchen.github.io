...
```
~ $ git clone https://github.com/goileolee/goileolee.github.io
clone完，在myblog下有個*goileolee.github.io*文件夾
```
1.3 創建一個index.html文件，作為測試文件

```
~ $ cd goileolee.github.io
...
1.5 驗證你的成果，打開你的瀏覽器，打開的的GitPages地址*http://goileolee.github.io*
...
3.2更換主題。你把你保存博客的文件夾打開，比如我的是`goileolee.github.io`把裡面的東西，全部清空,如果你跟我選的主題，一樣那麼打開頁面，點擊 `HomePage`就是這個開源主題，github的地址。當然你可以直接`Download`，我覺得git很方便。找到git的url。clone到 `goileolee.github.io`文件夾中
...
4.1 創建一個 `CNAME` 文件，放在你的根目錄下，打開填寫你的域名`goileo.top`

4.2 解析你的域名,A 記錄 ip 地址獲取：`ping goileolee.github.io`
